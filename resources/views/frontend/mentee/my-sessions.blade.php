                    ...
                    <div class="accordion">
                        ...
                        <div class="accordion-content">
                                        ...
                                        <div class="accordion" style="box-shadow:none; margin-bottom:10px;">
                                            ...
                                            <div class="accordion-content">
                                                ...
                                                <div
                                                    ...
                                                    <h4 style="margin: 0;">Assignments Given</h4>
                                                    ...
                                                    <input type="file" id="assignmentUploadInput"
                                                           accept="application/pdf" style="display:none;">
                                                    <input type="file" id="reuploadAssignmentFile"
                                                           accept="application/pdf" style="display:none;">
                                                </div>
                                                <table class="assignment-table">
                                                    ...
                                                    <tr>
                                                        ...
                                                        <th>Review Status</th>
                                                        <th>Approved / Reviewed Date</th>
                                                        <th>Reviewed Remarks</th>
                                                        <th>Actions</th>
                                                    </tr>
                                                    </thead>
                                                    <tbody>
                                                    @forelse($sessionAssignments as $assignment)
                                                        <tr @if($assignment->status === 'redo') style="background-color:#fff8e1;" @endif>
                                                            <td>{{ $assignment->original_file_name ?? 'Untitled Assignment' }}</td>

                                                            <td>{{ optional($assignment->uploader)->name ?? 'Unknown' }}</td>

                                                            <td>
                                                                {{ $assignment->submitted_at ? $assignment->submitted_at->format('d M Y') : '—' }}
                                                            </td>

                                                            <td>
                                                                @if($assignment->status === 'approved')
                                                                    <span
                                                                        class="status-chip status-done">Approved</span>
                                                                @elseif($assignment->status === 'rejected')
                                                                    <span
                                                                        class="status-chip status-failed">Rejected</span>
                                                                @elseif($assignment->status === 'redo')
                                                                    <span class="status-chip status-warning text-black">Redo</span>
                                                                @elseif($assignment->status === 'submitted')
                                                                    <span
                                                                        class="status-chip status-info">Submitted</span>
                                                                @else
                                                                    <span
                                                                        class="status-chip status-pending">{{ ucfirst($assignment->status) }}</span>
                                                                @endif
                                                            </td>

                                                            <td>
                                                                {{ $assignment->reviewed_at ? $assignment->reviewed_at->format('d M Y') : '—' }}
                                                            </td>

                                                            <td>
                                                                {{ $assignment->review_remarks ?? '—' }}
                                                            </td>
                                                            ...
                                                            <td style="text-align:center;">
                                                                ...
                                                                --}}{{-- Reupload Button if Status is Redo --}}{{--
                                                                @if($assignment->status === 'redo')
                                                                    <button
                                                                        class="btn btn-sm btn-warning reupload-assignment-btn"
                                                                        ...
                                                                        title="Your mentor requested changes. Click to reupload corrected file."
                            ...
                            <div id="session-accordion-container">
                                ...
                                @if($order->order_type == 'master_class')
                                    @foreach($order->masterClass->sessions as $session)
                                        @php
                                            $isPast = \Carbon\Carbon::now()->greaterThan(\Carbon\Carbon::parse($session->start_date_time));
                                            $shortDescription = \Illuminate\Support\Str::words($session->session_description, 50, '...');
                                        @endphp

                                        <div class="accordion" style="box-shadow:none; margin-bottom:10px;">
                                            <div class="accordion-header" onclick="toggleAccordion(this)"
                                                 style="font-size: 18px; font-weight:600;">
                                                {{ $session->title }} - {{ \Carbon\Carbon::parse($session->start_date_time)->format('d M Y') }}
                                                <span>+</span>
                                            </div>
                                            ...
                                            <div class="accordion-content">
                                                <table>
                                                    ...
                                                    <tr>
                                                        <td>{{ $session->title }}</td>
                                                        <td>{{ \Carbon\Carbon::parse($session->start_date_time)->format('d M Y') }} 📅</td>
                                                        <td>
                                                            @if($isPast)
                                                                <span class="status-chip status-done">Done</span>
                                                            @else
                                                        ...
                                                        <td>
                                                            ...
                                                            @if(\Illuminate\Support\Str::wordCount($session->session_description) > 50)
                                                                <a href="javascript:void(0);" class="read-more"
                                                                   data-full="{{ $session->session_description }}">Read more</a>
                                                            @endif
                                                        </td>
                                                        <td>
                                                            @php
                                                                $completedAssignments = \App\Models\OrderAssignment::where('order_id', $order->id)
                                                                    ->where('master_class_id', $order->masterClass->id)
                                                                    ->where('session_id', $session->id)
                                                                    ->where('status', 'approved')
                                                                    ->count();

                                                                $totalAssignments = \App\Models\OrderAssignment::where('order_id', $order->id)
                                                                    ->where('master_class_id', $order->masterClass->id)
                                                                    ->where('session_id', $session->id)
                                                                    ->count();
                                                            @endphp
                                                ...
                                                <div style="display: flex; justify-content: space-between; align-items: center;">
                                                    <h4 style="margin: 0;">Assignments Given by Organizer</h4>
                                                ...
                                                <table class="assignment-table">
                                                    ...
                                                    @forelse($sessionAssignments as $assignment)
                                                        <tr @if($assignment->status === 'redo') style="background-color:#fff8e1;" @endif>
                                                            <td>{{ $assignment->organizer_file_name ?? 'Untitled Assignment' }}</td>

                                                            <td>{{ optional($assignment->organizer)->name ?? 'Organizer' }}</td>

                                                            {{-- Organizer File --}}
                                                            <td>
                                                                @if($assignment->organizer_file_path)
                                                                    <a href="{{ asset('storage/'.$assignment->organizer_file_path) }}" target="_blank" class="btn btn-sm btn-outline-primary">
                                                            ...
                                                            <td>
                                                                @if($assignment->user_file_path)
                                                                    <a href="{{ asset('storage/'.$assignment->user_file_path) }}" target="_blank" class="btn btn-sm btn-outline-secondary">
                                                                        <i class="fas fa-download"></i> View
                                                                    </a>
                                                                @else
                                                                    —
                                                            ...
                                                            <td>
                                                                {{ $assignment->user_uploaded_at ? $assignment->user_uploaded_at->format('d M Y') : '—' }}
                                                            </td>
                                                            ...
                                                            <td>
                                                                {{ $assignment->organizer_reviewed_at ? $assignment->organizer_reviewed_at->format('d M Y') : '—' }}
                                                            </td>
                                                            ...
                                                            <td style="text-align:center;">
                                                                ...
                                                                @if($assignment->status === 'assigned')
                                                                    <button class="btn btn-sm btn-primary upload-assignment-btn"
                                                                            data-assignment-id="{{ $assignment->id }}"
                                                                            data-order-id="{{ $assignment->order_id }}"
                                                                            data-master-class-id="{{ $assignment->master_class_id }}"
                                                                            data-session-id="{{ $assignment->session_id }}">
...
{{--                                                                @if($assignment->status === 'redo')--}}
                                                                    {{--<button class="btn btn-sm btn-warning reupload-assignment-btn"
                                                                            data-assignment-id="{{ $assignment->id }}"
                                                                            data-order-id="{{ $assignment->order_id }}"
                                                                            data-master-class-id="{{ $assignment->master_class_id }}"
                                                                            data-session-id="{{ $assignment->session_id }}">
                                                                        <i class="fas fa-sync-alt"></i> Reupload
                                                                    </button>--}}
                                                                @endif
                                                            </td>
                                                        </tr>
                                                    @empty
                                                        <tr>
                                                            <td colspan="9" style="text-align:center; color:#888;">No assignments given yet.</td>
                                                        </tr>
                                                    @endforelse
                                                    </tbody>
                                                </table>
            ...
            <script>
                ...
                $(document).on('click', '.add-assignment-btn', function () {
                    ...
                    $('#assignmentUploadInput').off('change').on('change', function (e) {
                        ...
                        // Upload via AJAX
                        $.ajax({
                            url: "{{ route('frontend.upload.assignment') }}", // define in routes/web.php
                            type: 'POST',
                            data: formData,
                            contentType: false,
                            processData: false,
            ...
            <script>
                ...
                $(document).on('click', '.add-assignment-btn', function () {
                    ...
                    Swal.fire({
                        ...
                        html: `
                <div style="text-align:left;">
                    <label style="font-weight:600;">Select PDF File:</label><br>
                    <input type="file" id="organizerAssignmentFile" accept="application/pdf" class="swal2-input" style="width:100%;"><br>
                    <label style="font-weight:600;">Remarks (optional):</label><br>
                    <textarea id="organizerRemarks" class="swal2-textarea" placeholder="Enter notes for mentee..." rows="3"></textarea>
                </div>
            `,
                        showCancelButton: true,
                        confirmButtonText: 'Upload Assignment',
                        cancelButtonText: 'Cancel',
                        preConfirm: () => {
                            const file = $('#organizerAssignmentFile')[0].files[0];
                            const remarks = $('#organizerRemarks').val();
                        ...
                        if (result.isConfirmed) {
                            ...
                            $.ajax({
                                url: "{{ route('frontend.upload.assignment.organizer') }}",
                                type: 'POST',
                                data: formData,
                                contentType: false,
                                processData: false,
                                beforeSend: function () {
                                    Swal.fire({
                                        title: "Uploading...",
                                        text: "Please wait while the assignment is being uploaded.",
                                        allowOutsideClick: false,
                                        didOpen: () => Swal.showLoading()
                                    });
                                },
                                success: function (response) {
                                    Swal.fire({
                                        ...
                                        text: 'The assignment has been given to the mentee.',
                                        timer: 2000
                                    });
                                    location.reload();
                                },
                ...
                /**
                 * USER: Upload assignment (after completion)
                 */
                $(document).on('click', '.upload-assignment-btn', function () {
                    const assignmentId = $(this).data('assignment-id');
                    const orderId = $(this).data('order-id');
                    const masterClassId = $(this).data('master-class-id');
                    const sessionId = $(this).data('session-id');
                    ...
                    fileInput.on('change', function () {
                        ...
                        // AJAX upload
                        $.ajax({
                            url: "{{ route('frontend.user.upload.assignment') }}",
                            type: 'POST',
                            data: formData,
                            contentType: false,
                            processData: false,
                            beforeSend: function () {
                                Swal.fire({
                                    title: "Uploading...",
                                    text: "Please wait while your assignment is being submitted.",
                                    allowOutsideClick: false,
                                    didOpen: () => Swal.showLoading()
                                });
                            },
                ...
                /**
                 * USER: Reupload after review (Redo)
                 */
                $(document).on('click', '.reupload-assignment-btn', function () {
                    const assignmentId = $(this).data('assignment-id');
                    const orderId = $(this).data('order-id');
                    ...
                    Swal.fire({
                        ...
                        html: `
                <div style="text-align:left;">
                    <label style="font-weight:600;">Select New PDF File:</label><br>
                    <input type="file" id="redoFile" accept="application/pdf" class="swal2-input" style="width:100%;"><br>
                    <label style="font-weight:600;">Remarks (optional):</label><br>
                    <textarea id="redoRemarks" class="swal2-textarea" placeholder="Enter remarks for mentor..." rows="3"></textarea>
                </div>
            `,
                        showCancelButton: true,
                        confirmButtonText: 'Submit Reupload',
                        cancelButtonText: 'Cancel',
                        preConfirm: () => {
                            const file = $('#redoFile')[0].files[0];
                            const remarks = $('#redoRemarks').val();
                        ...
                        if (result.isConfirmed) {
                            ...
                            $.ajax({
                                ...
                                beforeSend: function () {
                                    Swal.fire({
                                        title: "Reuploading...",
                                        text: "Please wait while your updated assignment is being uploaded.",
                                        allowOutsideClick: false,
                                        didOpen: () => Swal.showLoading()
                                    });
                                },
