        ...
        <div class="row g-3">
            ...
            <div class="col-md-6 d-flex align-items-center justify-content-center min-vh-100">
                <div class="w-100" style="max-width: 340px;">
                    ...
                    <form method="POST" action="{{ route('verify-otp') }}" id="registrationForm">
                        @csrf
                        @if ($errors->any())
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="role" name="role" required>
                                @foreach($roles as $role)
                                    @if((isset($otpRoleId) && $otpRoleId == $role->id))
                                        <option value="{{ $role->id }}" {{ (isset($otpRoleId) && $otpRoleId == $role->id) ? 'selected' : '' }}>
                                            {{ $role->name }}
                                        </option>
                                    @endif
                                @endforeach
                        ...
                        <div class="text-center mb-3">
                            <div class="mb-2" style="color: #888;">We’ve sent a code to <strong>{{ $otpEmail ?? '9999 888 777' }}</strong></div>
