            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-end mb-3">
                        <a href="{{ route('master.skills.europa-skills') }}" class="btn btn-outline-primary rounded-pill">Europa Skills</a>
                        <a href="{{ route('master.skills.create') }}" class="btn btn-outline-primary rounded-pill">Add Skill</a>
                    </div>
                    ...
                    <table id="basic-datatable" class="table table-striped dt-responsive nowrap w-100">
                        ...
                        @foreach ($list as $data)
                            <tr>
                                <td> {{ $data->name }}</td>
                                <td>
                                    <a href="{{ route('master.skills.edit', $data) }}" class="btn btn-info btn-icon">
                                        <i data-lucide="pencil"></i>
                                    </a>
                                        <form action="{{ route('master.skills.destroy', $data->id) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
